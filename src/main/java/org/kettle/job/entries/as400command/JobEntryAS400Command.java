package org.kettle.job.entries.as400command;

import java.util.List;

import org.kettle.ui.job.entries.as400command.JobEntryAS400CommandDialog;
...
@JobEntry(id = "AS400Command", name = "JobEntryAS400Command.Name", description = "JobEntryAS400Command.Description", image = "as400command.svg", i18nPackageName = "org.kettle.job.entries.as400command", categoryDescription = "i18n:org.pentaho.di.job:JobCategory.Category.Utility")
...
public class JobEntryAS400Command extends JobEntryBase implements Cloneable, JobEntryInterface {
	...
	/**
	 * Get the name and port of the proxy server in the format serverName[:port]. If
	 * no port is specified, a default will be used.
	 * 
	 * @return
	 */
	...
	public Result execute(final Result result, int nr) throws KettleException {
		...
		try {
			// Create proxy server
			String proxyServer = this.getProxyServer(environmentSubstitute(proxyHost),
					environmentSubstitute(proxyPort));
			...
			if (success) {
				if (isBasic()) {
					...
					logBasic(BaseMessages.getString(PKG, "JobEntryAS400Command.Log.CommandSuccess", serverString,
							commandString));
				}
				result.setNrErrors(0);
				result.setResult(true);
			} else {
				logError(BaseMessages.getString(PKG, "JobEntryAS400Command.Log.CommandFailure", serverString,
						commandString));
			...
			logError(
					BaseMessages.getString(PKG, "JobEntryAS400Command.Log.CommandFailure", serverString, commandString),
					e);
		} finally {
			try {
	...
	public void loadXML(Node entrynode, List<DatabaseMeta> databases, List<SlaveServer> slaveServers, Repository rep,
		...
		try {
			...
			throw new KettleXMLException(BaseMessages.getString(PKG, "JobEntryAS400Command.Exception.UnableToReadXML"),
					xe);
		}
	}
	...
	public void loadRep(Repository repository, IMetaStore metaStore, ObjectId id_jobentry, List<DatabaseMeta> databases,
		...
		try {
			...
			password = Encr.decryptPasswordOptionallyEncrypted(
					repository.getJobEntryAttributeString(id_jobentry, TAG_PASSWORD));
			...
			throw new KettleException(
					BaseMessages.getString(PKG, "JobEntryAS400Command.Exception.UnableToReadRepository", id_jobentry),
					dbe);
	...
	public void check(List<CheckResultInterface> remarks, JobMeta jobMeta, VariableSpace space, Repository repository,
		...
		JobEntryValidatorUtils.andValidator().validate(this, TAG_SERVER, remarks,
				AndValidator.putValidators(JobEntryValidatorUtils.notBlankValidator()));
		JobEntryValidatorUtils.andValidator().validate(this, TAG_USER, remarks,
				AndValidator.putValidators(JobEntryValidatorUtils.notBlankValidator()));
		JobEntryValidatorUtils.andValidator().validate(this, TAG_PASSWORD, remarks,
				AndValidator.putValidators(JobEntryValidatorUtils.notNullValidator()));
		JobEntryValidatorUtils.andValidator().validate(this, TAG_COMMAND, remarks,
				AndValidator.putValidators(JobEntryValidatorUtils.notBlankValidator()));
	}
